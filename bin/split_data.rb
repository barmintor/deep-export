...
CSV.foreach('docs/data/deep-export.csv') do |row|
  if headers
...
headers.length.times do |i|
  CSV.open("docs/data/transform/#{headers[i]}.csv", "wb") do |csv|
    csv << ['id', 'label']
...
join_tables.each do |join_to|
  ...
  CSV.open("docs/data/transform/join_#{join_to}.csv", "wb") do |csv|
    csv << ["deep_id", "#{join_id}","uncertain"]
  end
end

deep_headers = headers.reject {|h| join_tables.include?(h)}
CSV.open("docs/data/transform/deep.csv", "wb") do |csv|
  csv << deep_headers
end
...
CSV.foreach('docs/data/deep-export.csv') do |row|
  if headers
    row.length.times do |i|
      ...
      if join_tables.include? headers[i]
        ...
        tmp.each do |tmp_val|
          ...
          CSV.open("docs/data/transform/join_#{join_to}.csv", "ab") do |csv|
            csv << [row[0], ix.to_s, !!q]
          end
        end
      end
    ...
    CSV.open("docs/data/transform/deep.csv", "ab") do |csv|
      csv << filtered
    end
  else
    headers = row
